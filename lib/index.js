...
Nightmare.prototype.run = function(callback, teardown) {
  var self = this;
  if (typeof callback === "Boolean") teardown = callback;
  if (typeof teardown === "undefined") teardown = true;
  debug('run');
  this.setup(function () {
    setTimeout(next, 0);
    function next(err) {
      ...
      if (!item) {
        if (teardown) self.teardownInstance();
